...
function initializeNetwork(){
	...
	document.getElementById("nnPlaceHolder").innerHTML = "<br>"+ JSON.stringify(nn, null, '\t');
...
function editNN(){
	for(var i  = 0; i<nn.hiddenLayers.length; i++){
		layer = nn.hiddenLayers[i];
		for(var n = 0; n<layer.length; n++){
			neuron = layer[n];
			for(var w = 0; w< neuron.weights.length; w++){
				neuron.weights[w] += Math.random()*10000;
			}
		}
...
function drawNodes(){
	...
	for(var i = 0; i<nn.inputs.length; i++){
		...
		for(var k = 0; k<nn.hiddenLayers[0].length;k++){
			...
			ctxN.stroke();
			ctxN.fillStyle = "#000000"
			drawLabel(ctxN, Math.floor(nn.hiddenLayers[0][k].weights[i]*100)/100, {"x":colWidth/2+20,"y":rowHeight*(i+.5)},{"x":colWidth/2+colWidth*(1),"y":Height/nn.hiddenLayers[0].length*(k+.5)},"left",60)
			//ctxN.fillText(nn.hiddenLayers[0][k].weights[0],colWidth/2+100,Height/nn.hiddenLayers[0].length*(k+.5))
